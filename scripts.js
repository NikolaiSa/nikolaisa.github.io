$(document).ready(function(){
    ...
    var printSize = function(appHeading) {
        ...
        $.getJSON("https://itunes.apple.com/lookup/json?id=" + appid + "&callback=?", function( json ) {
            ...
            appHeading.innerHTML = appHeading.innerHTML + "<td>"+(json.results[0].fileSizeBytes/1024/1024).toPrecision(2) + " MB "+"<\/td>" + "<td>"+json.results[0].version+"<\/td> <td>"+json.results[0].averageUserRatingForCurrentVersion+"<\/td> <td>"+json.results[0].minimumOsVersion+"<\/td> <td>"+json.results[0].userRatingCount+"<\/td>" + "<td>"+json.results[0].userRatingCountForCurrentVersion+"<\/td>";
        });
